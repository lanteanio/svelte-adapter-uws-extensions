...
// convergence/seq-regression checks (sim.js, checked once at end-of-run over
...
//
// This is the extensions mirror of the adapter's runtime module, kept as a COPY
// (not an import) the same way invariants.js is: the sim tier builds and tests
// against the PUBLISHED adapter, so importing an adapter-internal module would
// couple releases. The predicate bodies are byte-identical to the adapter's;
// the runners that feed them are runRedisSim / runPgSim.
//
 ...
 * channel (the per-instance wire faults or the cross-instance relay faults),
 * because a legitimate interleaving under either one is exactly what the guard
 * exists to excuse. Pure: reads only the passed-in spec objects.
 *
 ...
 * bug. A run with more than one instance publishing to the same topic
 ...
 * received no readable frame on a topic is expected, not a bug. A run that lost
 * an instance mid-flight would also legitimately drop in-flight deliveries, so
 * `disrupted` suppresses it too (unused by the current runners, which never
 * kill an instance; kept for parity with the adapter's cluster runner).
 *
 * @param {FaultClasses | undefined} faults
 * @returns {boolean}
 */
 ...
 * PUBLISH time (not end-of-run), so only clients actually eligible for that
 * broadcast are checked. Cross-instance delivery through the store relay is
 * DEFERRED (it lands on a later scheduler round), so a client that unsubscribes
 * inside the relay window legitimately receives nothing; the runners
 * pre-restrict `publishLog` to subscribers still on the topic at end-of-run
 * before calling here (the same restriction the adapter's cluster runner
 * applies) - this predicate itself only reads the log it is handed.
 ...
 * drop / corrupt / disruption (see guardedForStarvation). A publish
